...
class Model(tpu_utils.Model):
    def __init__(self, *, model_name, betas: np.ndarray, model_mean_type: str, model_var_type: str, loss_type: str, num_classes: int, dropout: float, randflip):
        self.model_name = model_name
        self.diffusion = GaussianDiffusion2(betas=betas, model_mean_type=model_mean_type, model_var_type=model_var_type, loss_type=loss_type)
    ...
    def _denoise(self, x, t, y, dropout):
        ...
        if self.model_name == 'unet2d16b2':  # 35.7M
            return unet.model(x, t=t, y=y, name='model', ch=128, ch_mult=(1, 2, 2, 2), num_res_blocks=2, attn_resolutions=(16,), out_ch=out_ch, num_classes=self.num_classes, dropout=dropout)
    ...
    def samples_fn(self, dummy_noise, y):
        return {'samples': self.diffusion.p_sample_loop(denoise_fn=functools.partial(self._denoise, y=y, dropout=0), shape=dummy_noise.shape.as_list(), noise_fn=tf.random_normal)}
    ...
    def bpd_fn(self, x, y):
        total_bpd_b, terms_bpd_bt, prior_bpd_b, mse_bt = self.diffusion.calc_bpd_loop(denoise_fn=functools.partial(self._denoise, y=y, dropout=0), x_start=x)
        return {'total_bpd': total_bpd_b, 'terms_bpd': terms_bpd_bt, 'prior_bpd': prior_bpd_b, 'mse': mse_bt}


def _load_model(kwargs, ds):
    return Model(model_name=kwargs['model_name'], betas=get_beta_schedule(kwargs['beta_schedule'], beta_start=kwargs['beta_start'], beta_end=kwargs['beta_end'], num_diffusion_timesteps=kwargs['num_diffusion_timesteps']), model_mean_type=kwargs['model_mean_type'], model_var_type=kwargs['model_var_type'], loss_type=kwargs['loss_type'], num_classes=ds.num_classes, dropout=kwargs['dropout'], randflip=kwargs['randflip'])
...
def evaluation(model_dir, tpu_name, bucket_name_prefix, once=False, dump_samples_only=False, total_bs=256,
...
if __name__ == '__main__':
    ...
    num_classes = tf.size(tf.unique(fakeY))

    # Create the model for the reverse diffusion process as described by Ho et al. 2020 DPPM paper (arxiv:2006.11239)
    # reverseDiffusionModel = models.unet.model(name="here", betas=betas, model_mean_type='eps', model_var_type='fixed', loss_type='mse', num_classes=num_classes, dropout=0.1, randflip=1)
    model = Model(model_name='name', betas=betas, model_mean_type='eps', model_var_type='fixed', loss_type='mse', num_classes=10, dropout=0., randflip=True)
